...
class Net(torch.nn.Module): 
    def __init__(self, flag): 
        ...
        self.dense_0 = torch.nn.Linear(56, 32, bias=False) 
        ...
        # self.dropout_0 = torch.nn.Dropout(0.90) 
        # self.dropout_1 = torch.nn.Dropout(0.90) 
        self.data = X_data 
        self.flag = flag 
        
    def forward(self, x): 
        if self.flag == "1V":
            label_0 = self.data[ 3,:].reshape(1,1,7).float()   # 1V
            label_1 = self.data[ 4,:].reshape(1,1,7).float()   # 1V
            label_2 = self.data[13,:].reshape(1,1,7).float()   # 1V 
        else:
            label_0 = self.data[ 3,:].reshape(1,1,7).float()   # 2V
            label_1 = self.data[11,:].reshape(1,1,7).float()   # 2V
            label_2 = self.data[13,:].reshape(1,1,7).float()   # 2V 
...
if __name__ == "__main__":
    model = Net("1V") 
    coord_tensor = torch.randn([1,1,7]) 
